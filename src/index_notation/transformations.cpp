    ...
    IndexStmt Reorder::apply(IndexStmt stmt, string *reason) const {
        ...
        match(stmt,
              std::function<void(const ForallNode *)>([&](const ForallNode *op) {
                  bool matches = std::find(getreplacepattern().begin(), getreplacepattern().end(), op->indexVar) !=
                                 getreplacepattern().end();
                  if (matches) {
        ...
        if (matchFailed || currentOrdering.size() != getreplacepattern().size()) {
            *reason =
                    "The foralls of reorder pattern: " + util::join(getreplacepattern()) + " were not directly nested.";
            return IndexStmt();
        }
...
// class Precompute
    ...
    IndexStmt Precompute::apply(IndexStmt stmt, std::string *reason) const {
        ...
        struct PrecomputeRewriter : public IndexNotationRewriter {
            ...
            Assignment getConsumerAssignment(IndexStmt stmt, TensorVar &ws) {
                ...
                switch (rel) {
                    ...
                    case equal:
                        a = Assignment(a.getLhs(), a.getRhs());
                        break;//return RVars;//reductionVars.clear(); return reductionVars;//return RVars;// // = OR +=
                }
        ...
        struct RedundentVisitor : public IndexNotationVisitor {
            ...
            RedundentVisitor(std::vector<Assignment> &to_change, const ProvenanceGraph &provGraph) : to_change(
                    to_change), provGraph(provGraph), ctx_num(0) {}
                      ...
                      std::function<void(const AccessNode *)>([&](const AccessNode *op) {
                          for (auto &var: op->indexVars) {
                              for (auto &svar: ctx_stack) {
                                  if ((provGraph.getUnderivedAncestors(var)[0] ==
                                       provGraph.getUnderivedAncestors(svar)[0]) && svar != var) {
                                      has_sibling = true;
                                  }
                              }
                          }
...
// class ForAllReplace
    ...
    ForAllReplace::ForAllReplace(std::vector<IndexVar> pattern, std::vector<IndexVar> replacement) : content(
            new Content) {
...
// class Parallelize
    ...
    Parallelize::Parallelize(IndexVar i, ParallelUnit parallel_unit, OutputRaceStrategy output_race_strategy) : content(
            new Content) {
        content->i = i;
                ...
                if (foralli.getIndexVar() == i) {
                    ...
                    if (parallelize.getOutputRaceStrategy() == OutputRaceStrategy::Temporary &&
                        ...
                        for (auto assignment: precomputeAssignments) {
                            ...
                            TensorVar w(string("w_") + ParallelUnit_NAMES[(int) parallelize.getParallelUnit()],
                                        Type(assignment->lhs.getDataType(), {Dimension(i)}), taco::dense);

                            // rewrite producer to write to temporary, mark producer as parallel
                            IndexStmt producer = ReplaceReductionExpr(
                                    map<Access, Access>({{assignment->lhs, w(i)}})).rewrite(precomputed_stmt);
                            taco_iassert(isa<Forall>(producer));
                            Forall producer_forall = to<Forall>(producer);
                            producer = forall(producer_forall.getIndexVar(), producer_forall.getStmt(),
                                              parallelize.getParallelUnit(), parallelize.getOutputRaceStrategy(),
                                              foralli.getUnrollFactor());
                            ...
                            IndexStmt consumer = forall(i, Assignment(assignment->lhs, w(i), assignment->op),
                                                        reductionUnit, OutputRaceStrategy::ParallelReduction);
                    ...
                    stmt = forall(i, foralli.getStmt(), parallelize.getParallelUnit(),
        ...
        struct InlineTemporaries : public IndexNotationRewriter {
            ...
            void visit(const AssignmentNode *op) {
                ...
                const Access rhsAccess = isa<Access>(op->rhs) ? to<Access>(op->rhs)
                                                              : (isa<Cast>(op->rhs) &&
                                                                 isa<Access>(to<Cast>(op->rhs).getA()))
...
// Autoscheduling functions
    ...
    IndexStmt parallelizeOuterLoop(IndexStmt stmt) {
        ...
        if (should_use_CUDA_codegen()) {
            ...
            parallelized256 = Parallelize(i1, ParallelUnit::GPUBlock, OutputRaceStrategy::NoRaces).apply(
                    parallelized256, &reason);
            if (parallelized256 == IndexStmt()) {
                return stmt;
            }

            parallelized256 = Parallelize(i2, ParallelUnit::GPUThread, OutputRaceStrategy::NoRaces).apply(
                    parallelized256, &reason);
            if (parallelized256 == IndexStmt()) {
                return stmt;
            }
            ...
            IndexStmt parallelized = Parallelize(forall.getIndexVar(), ParallelUnit::CPUThread,
                                                 OutputRaceStrategy::NoRaces).apply(stmt, &reason);
...
// TODO Temporary function to insert workspaces into SpMM kernels
            ...
            void visit(const ForallNode *node) {
                ...
                if (pattern == foralli.getIndexVar()) {
                    ...
                    for (auto i = replacement.rbegin(); i != replacement.rend(); ++i) {
                        cout << *i << endl;
                        stmt = forall(*i,
                                      stmt); // Wrong, because we have to rewrite its content and change the indexVar
                        cout << stmt << endl;
                    }
                    return;
                }
    ...
    struct MultiForallReplace::Content {
        std::vector<IndexVar> pattern;
        IndexVar replacement;
    };

    MultiForallReplace::MultiForallReplace() : content(nullptr) {
    }

    MultiForallReplace::MultiForallReplace(std::vector<IndexVar> pattern, IndexVar replacement) : content(new Content) {
        taco_iassert(!pattern.empty());
        content->pattern = pattern;
        content->replacement = replacement;
    }

    std::vector<IndexVar> MultiForallReplace::getPattern() const {
        return content->pattern;
    }

    IndexVar MultiForallReplace::getReplacement() const {
        return content->replacement;
    }

    void MultiForallReplace::print(std::ostream &os) const {
        os << "MultiForallReplace(" << util::join(getPattern()) << ", " << getReplacement() << ")";
    }

    std::ostream &operator<<(std::ostream &os, const MultiForallReplace &forallreplace) {
        forallreplace.print(os);
        return os;
    }

    IndexStmt MultiForallReplace::apply(IndexStmt stmt, string *reason) const {
        INIT_REASON(reason);
        ...
        struct ForAllReplaceRewriter : public IndexNotationRewriter {
            ...
            IndexStmt forallreplace(IndexStmt stmt) {
                ...
                cout << replaced << endl;
                // Precondition: Did not find pattern
                vector<IndexVar> replacement;
                replacement.push_back(transformation.getReplacement());
                if (replaced == stmt) {
                    *reason = "The pattern of ForAlls: " +
                              util::join(transformation.getPattern()) +
                              " was not found while attempting to replace with: " +
            ...
            void visit(const ForallNode *node) {
                ...
                // Precondition: pattern size is larger than one
                vector<IndexVar> pattern = transformation.getPattern();
                IndexVar replacement = transformation.getReplacement();
                bool push = false;
                // Push current level
                if (ctx_stack.size() == 0) {
                    if (pattern[0] == currentIndex && isa<Forall>(foralli.getStmt())) {
                        // Child is directly nested and is the first match
                        ctx_stack.push_back(0);
                        push = true;
                    }
                } else {
                    int currentLevel = ctx_stack.back();
                    if (pattern[(currentLevel + 1) % patternLength] == currentIndex && isa<Forall>(foralli.getStmt())) {
                        ctx_stack.push_back((currentLevel + 1) % patternLength);
                        push = true;
                    } else {
                        // Roll back
                        if (currentLevel < patternLength - 1) {
