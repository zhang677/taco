...
std::string CodeGen::printWsFuncs(std::map<std::string, std::pair<int, std::string>> wsvars) {
  ...
  for (auto& item: wsvars) {
    ...
    ret << "}" <<endl;
    ret << "int Merge(int32_t** COO_crd, "<<type<<"* COO_vals, int32_t COO_size, "<<clsname<<"* accumulator, int32_t accumulator_size) {\n";
    ret << "    if (COO_size == 0) {\n";
    ...
    ret << "    int content_pointer = 0;\n";
    ret << "    int target_pointer = 0;\n";
    ret << "    "<<clsname<<" tmp_con;\n";
